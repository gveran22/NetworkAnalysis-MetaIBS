...
plotting_filtering_comparison <- function(df, df2, tax){
  ...
  df_long <- df %>%
  dplyr::filter(agg_level == tax) %>%
  pivot_longer(cols = c(ASVs_agglo, ASVs_IBS, ASVs_filtered_IBS, ASVs_H, ASVs_filtered_H),
               names_to = "stage", values_to = "ASV_count") %>%
  mutate(stage = factor(stage, levels = c("ASVs_agglo", "ASVs_IBS","ASVs_filtered_IBS", 
