...
thresh_H.gl <- mean(meta_H.gl) + 0.3*sd(meta_H.gl)
thresh_IBS.gl <- mean(meta_IBS.gl) + 0.3*sd(meta_IBS.gl)
thresh.gl <- (thresh_H.gl / var(meta_H.gl) + thresh_IBS.gl / var(meta_IBS.gl)) / 
             (1 / var(meta_H.gl) + 1 / var(meta_IBS.gl))

thresh_H.mb <- mean(meta_H.mb) + 0.3*sd(meta_H.mb)
thresh_IBS.mb <- mean(meta_IBS.mb) + 0.3*sd(meta_IBS.mb)
thresh.mb <- (thresh_H.mb / var(meta_H.mb) + thresh_IBS.mb / var(meta_IBS.mb)) / 
             (1 / var(meta_H.mb) + 1 / var(meta_IBS.mb))

thresh_H.slr <- mean(meta_H.slr) + 0.3*sd(meta_H.slr)
thresh_IBS.slr <- mean(meta_IBS.slr) + 0.3*sd(meta_IBS.slr)
...
### Common associations
net_g <- network_construct_comparison(prop_H.gl, prop_IBS.gl, "proportionality", thresh = 0.3)
asso_H.g <- net_g$input$assoMat1
asso_IBS.g <- net_g$input$assoMat2


net_mb <- network_construct_comparison(prop_H.mb, prop_IBS.mb, "proportionality", thresh = 0.3)
asso_H.mb <- net_mb$input$assoMat1
asso_IBS.mb <- net_mb$input$assoMat2

net_slr <- network_construct_comparison(prop_H.slr, prop_IBS.slr, "proportionality", thresh = 0.3)
