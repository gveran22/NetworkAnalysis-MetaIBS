...
knitr::opts_chunk$set(echo= TRUE, warning = FALSE, message = FALSE, fig.path="~/MetaIBS/outputs/network-comparison/Individual/plots/Family/")
```
...
properties_H.gl <- lapply(datasets_names, function(x) {
  load(file=file.path(path.properties, agg_level, paste0("NetProp_",x,".RData")))
  objects <- ls()
  return(get(objects[1]))
  })

properties_IBS.gl <- lapply(datasets_names, function(x) {
  load(file=file.path(path.assoc_mat, agg_level, paste0("NetProp_",x,".RData")))
  objects <- ls()
