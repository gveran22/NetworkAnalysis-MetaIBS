...
matrices_H.gl <- lapply(datasets_names, function(x) {
  tryCatch({
    ...
    objects <- ls()  # Get the loaded objects
    return(get(objects[1]))  # Return the first object
  }, error = function(e) {
    # Handle errors (e.g., empty files) by skipping them
    message(paste("Skipping file:", x, "due to error:", e$message))
    return(NULL)  # Return NULL for skipped files
  })
})

# Filter out the NULL entries from the result
matrices_H.gl <- Filter(Negate(is.null), matrices_H.gl)
...
matrices_IBS.gl <- lapply(datasets_names, function(x) {
  tryCatch({
    ...
    load(file = file.path(path.assoc_mat, agg_level, paste0("AssocMat_", x, ".RData")))
    objects <- ls()  # Get the loaded objects
    return(get(objects[4]))  # Return the first object
  }, error = function(e) {
    # Handle errors (e.g., empty files) by skipping them
    message(paste("Skipping file:", x, "due to error:", e$message))
    return(NULL)  # Return NULL for skipped files
...
# Filter out the NULL entries from the result
matrices_IBS.gl <- Filter(Negate(is.null), matrices_IBS.gl)
...
matrices_H.mb <- lapply(datasets_names, function(x) {
  tryCatch({
    ...
    load(file = file.path(path.assoc_mat, agg_level, paste0("AssocMat_", x, ".RData")))
    objects <- ls()  # Get the loaded objects
    return(get(objects[2]))  # Return the first object
  }, error = function(e) {
    # Handle errors (e.g., empty files) by skipping them
    message(paste("Skipping file:", x, "due to error:", e$message))
    return(NULL)  # Return NULL for skipped files
...
# Filter out the NULL entries from the result
matrices_H.mb <- Filter(Negate(is.null), matrices_H.mb)
...
matrices_IBS.mb <- lapply(datasets_names, function(x) {
  tryCatch({
    ...
    load(file = file.path(path.assoc_mat, agg_level, paste0("AssocMat_", x, ".RData")))
    objects <- ls()  # Get the loaded objects
    return(get(objects[5]))  # Return the first object
  }, error = function(e) {
    # Handle errors (e.g., empty files) by skipping them
    message(paste("Skipping file:", x, "due to error:", e$message))
    return(NULL)  # Return NULL for skipped files
...
# Filter out the NULL entries from the result
matrices_IBS.mb <- Filter(Negate(is.null), matrices_IBS.mb)
...
matrices_H.slr <- lapply(datasets_names, function(x) {
  tryCatch({
    ...
    load(file = file.path(path.assoc_mat, agg_level, paste0("AssocMat_", x, ".RData")))
    objects <- ls()  # Get the loaded objects
    return(get(objects[3]))  # Return the first object
  }, error = function(e) {
    # Handle errors (e.g., empty files) by skipping them
    message(paste("Skipping file:", x, "due to error:", e$message))
    return(NULL)  # Return NULL for skipped files
...
# Filter out the NULL entries from the result
matrices_H.slr <- Filter(Negate(is.null), matrices_H.slr)
...
matrices_IBS.slr <- lapply(datasets_names, function(x) {
  tryCatch({
    ...
    load(file = file.path(path.assoc_mat, agg_level, paste0("AssocMat_", x, ".RData")))
    objects <- ls()  # Get the loaded objects
    return(get(objects[6]))  # Return the first object
  }, error = function(e) {
    # Handle errors (e.g., empty files) by skipping them
    message(paste("Skipping file:", x, "due to error:", e$message))
    return(NULL)  # Return NULL for skipped files
...
# Filter out the NULL entries from the result
matrices_IBS.slr <- Filter(Negate(is.null), matrices_IBS.slr)
