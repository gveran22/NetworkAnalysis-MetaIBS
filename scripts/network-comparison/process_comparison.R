...
for (path_suffix in names(analysis_variables)) {
  path.phylobj    <- file.path(path.root, "build/Agglomeration/Combined", path_suffix)
  path.outputs <- file.path(path.root, "outputs/network-comparison/Combined", path_suffix)
  path.phylobj_sep <- file.path(path.outputs , "phyloseq_IBS")
  path.filt_phy <- file.path(path.outputs , "filtered_otus")
  path.spiec_easi <- file.path(path.outputs , "spiec-easi_results")
  path.assoc_mat <- file.path(path.outputs , "association_matrices")
  ...
  datasets_names  <- analysis_variables[[path_suffix]]
