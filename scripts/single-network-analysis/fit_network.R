...
fit_network <- function(physeq_name, agg_level){
  ...
  se.gl <- spiec.easi(physeq_filt, method='glasso', nlambda=50,
                      lambda.min.ratio=1e-3, lambda.log=FALSE,
                      pulsar.select=TRUE, pulsar.params=pargs)
  
  se.mb <- spiec.easi(physeq_filt, method='mb', nlambda=50,
                      lambda.min.ratio=1e-3, lambda.log=FALSE,
                      pulsar.select=TRUE, pulsar.params=pargs)
  
  ranks <- rank.slr(physeq_filt)
  
  se.slr <- spiec.easi(physeq_filt , method='slr', nlambda=50,
                       lambda.min.ratio=1e-3, r=ranks, lambda.log=FALSE,
                       pulsar.select=TRUE, pulsar.params=pargs)
